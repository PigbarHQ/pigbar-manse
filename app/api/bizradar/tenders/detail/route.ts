import { bizRadarErrorPayload, bizRadarErrorStatus, fetchTenderDetail, type TenderBusinessType } from "@/src/lib/bizradar";
...
export async function GET(request: Request) {
  try {
    ...
    if (!bidNoticeNo) {
      return Response.json({ error: "bidNoticeNo is required" }, { status: 400 });
    }

    return Response.json(await fetchTenderDetail({
