...
test("Reader exposes simplified Portrait Book MVP UI", () => {
  ...
  assert.match(workspaceSource, /Blind/);
  assert.match(workspaceSource, /Future/);
  ...
  assert.match(workspaceSource, /onSubmit/);
  assert.match(workspaceSource, /onContinue/);
  assert.match(workspaceSource, /onSelect\(experience\)/);
  assert.doesNotMatch(workspaceSource, /setExperienceId\(DEFAULT_BLUEPRINT_EXPERIENCE\.id\)/);
  assert.doesNotMatch(workspaceSource, /선택됨:/);
  assert.doesNotMatch(workspaceSource, /미리보기/);
  assert.doesNotMatch(workspaceSource, /이 출판식으로 시작하기/);
  assert.match(workspaceSource, /Experience 다시 선택/);
  assert.match(workspaceSource, /Blueprint Input/);
  assert.match(workspaceSource, /생년월일과 출생시간을 기준으로 Portrait Book을 생성합니다/);
  assert.match(workspaceSource, /생성 방식/);
  assert.match(workspaceSource, /샘플로 보기/);
  assert.match(workspaceSource, /GPT로 생성/);
  assert.match(workspaceSource, /샘플 책 보기/);
  assert.match(workspaceSource, /ExperienceInput/);
  assert.match(workspaceSource, /GlassInput/);
  assert.match(workspaceSource, /PaperInput/);
  assert.match(workspaceSource, /LuxuryInput/);
  assert.match(workspaceSource, /ClassicInput/);
  assert.match(workspaceSource, /CinematicInput/);
  assert.match(workspaceSource, /이 명조로 책 만들기/);
  assert.match(workspaceSource, /generationMode === "sample"/);
  assert.match(workspaceSource, /window\.setTimeout\(resolve, 700\)/);
  assert.match(workspaceSource, /fetch\("\/api\/manse"/);
  assert.match(workspaceSource, /blindCompiler: manseData\.blindCompiler/);
  assert.match(workspaceSource, /futureCompiler: manseData\.futureCompiler/);
  assert.match(workspaceSource, /fetch\("\/api\/blueprint"/);
  assert.match(workspaceSource, /manuscriptSource: "Sample"/);
  assert.match(workspaceSource, /sampleBlindAnalysis/);
  assert.match(experiencesSource, /Warm Book Edition/);
  assert.match(experiencesSource, /Glass Blueprint Edition/);
  assert.match(experiencesSource, /Luxury Edition/);
  assert.match(experiencesSource, /Cinematic Illustration Edition/);
  assert.match(experiencesSource, /cinematic-illustration/);
  assert.match(experiencesSource, /cinematic landscape, gold light, quiet cover/);
  assert.match(experiencesSource, /ivory paper, ink, editorial desk/);
  assert.match(experiencesSource, /velvet, seal, gold leaf/);
  assert.match(experiencesSource, /원고 접수/);
  assert.match(experiencesSource, /각인/);
  assert.match(experiencesSource, /input:/);
  assert.match(experiencesSource, /backdrop-blur-xl/);
  assert.match(experiencesSource, /hover:shadow-\[0_0_18px_rgba\(240,184,91,0\.12\)\]/);
  assert.match(experiencesSource, /bg-\[#0d0905\]\/70/);
  assert.match(workspaceSource, /border-r border-t border-\[#b8a98f\]\/65/);
  assert.match(workspaceSource, /BP/);
  assert.match(sampleSource, /움직임을 길로 바꾸는 사람/);
  assert.match(sampleSource, /길을 만든다/);
  assert.match(sampleSource, /narrativeLens: "River"/);
  assert.match(sampleSource, /pageNo: 5/);
  assert.match(sampleSource, /finalCounsel/);
  assert.match(sampleBlindSource, /명조 확정/);
  assert.match(sampleBlindSource, /적천수/);
  assert.match(sampleBlindSource, /근원은 존재한다/);
  assert.match(sampleBlindSource, /정체보다 유통이 자연스럽다/);
  assert.match(readerSource, /아직 출판된 책이 없습니다/);
  assert.match(readerSource, /이 사람의 책을 쓰고 있습니다/);
  assert.match(readerSource, /명조를 읽고 있습니다/);
  assert.match(readerSource, /기준을 찾고 있습니다/);
  assert.match(readerSource, /문장을 다듬고 있습니다/);
  assert.match(readerSource, /출판을 준비하고 있습니다/);
  assert.match(readerSource, /GoldenLayerAnimation/);
  assert.match(readerSource, /PaperLayerAnimation/);
  assert.match(readerSource, /당신의 책이 완성되었습니다/);
  assert.match(readerSource, /책 읽기/);
  assert.match(readerSource, /다시 만들기/);
  assert.match(readerSource, /BookCoverCard/);
  assert.match(readerSource, /PagedPortraitReader/);
  assert.match(readerSource, /BlindCompilerDebug/);
  assert.match(readerSource, /Blind Compiler JSON/);
  assert.match(readerSource, /Signals Summary/);
  assert.match(readerSource, /FutureCompilerDebug/);
  assert.match(readerSource, /Future Compiler JSON/);
  assert.match(readerSource, /Future Signals Summary/);
  assert.match(readerSource, /Monthly Timing Index/);
  assert.match(readerSource, /High Relation Months/);
  assert.match(readerSource, /Wealth Timing Months/);
  assert.match(readerSource, /Officer Timing Months/);
  assert.match(readerSource, /Resource Timing Months/);
  assert.match(readerSource, /Movement Timing Months/);
  assert.match(readerSource, /DecisionCompilerDebug/);
  assert.match(readerSource, /Decision Compiler JSON/);
  assert.match(readerSource, /Decision Summary/);
  assert.match(readerSource, /opportunityScore/);
  assert.match(readerSource, /readerStatus/);
  assert.match(readerSource, /Decision Evidence Sample/);
  assert.match(readerSource, /decisionSummaryIndex/);
  assert.match(readerSource, /ReaderReportDebug/);
  assert.match(readerSource, /Template Reader Report/);
  assert.match(readerSource, /Top Opportunities/);
  assert.match(readerSource, /Top Risks/);
  assert.match(readerSource, /Domain Reports/);
  assert.match(readerSource, /Month Highlights/);
  assert.match(readerSource, /Caution Notes/);
  assert.match(readerSource, /currentPage/);
  assert.match(readerSource, /portrait\.pages\[currentPage\]/);
  assert.match(readerSource, /PortraitPaperPage/);
  assert.match(readerSource, /Page \{page\.pageNo\}/);
  assert.match(readerSource, /← 이전 장/);
  assert.match(readerSource, /다음 장 →/);
  assert.match(readerSource, /마지막 당부 보기/);
  assert.match(readerSource, /책 닫기/);
  assert.doesNotMatch(readerSource, /portrait\.pages\.map\(\(page\)/);
  assert.match(readerSource, /paperSurfaceClassName/);
  assert.match(readerSource, /min-h-\[520px\]/);
  assert.match(readerSource, /bg-\[length:26px_26px,100%_100%\]/);
  assert.match(readerSource, /Portrait Book/);
  assert.match(readerSource, /마지막 당부/);
  assert.match(readerSource, /min-h-\[460px\]/);
  assert.match(readerSource, /BlindAnalysisAppendix/);
  assert.match(readerSource, /전문 분석 보기/);
  assert.match(readerSource, /Appendix/);
  assert.match(readerSource, /Blind Classical Analysis/);
  assert.match(readerSource, /근거/);
  assert.match(readerSource, /구조/);
  assert.match(readerSource, /해석/);
